 ...
 *-----------------------------------------------------------------------------
 *
 * $Id: wi_stuff.c,v 1.14 1999/10/27 18:35:50 cphipps Exp $
 *
...
// "red P[1..MAXPLAYERS]"
static const patch_t  * facebackp;

// Name graphics of each level (centered)
static const patch_t  **lnames = NULL;
...
OVERLAY void WI_drawLF(void)
{
  ...
  V_DrawMemPatch((320 - SHORT(lnames[wbs->last]->width))/2, y, 
		 FB, lnames[wbs->last], NULL, VPT_STRETCH);

  // draw "Finished!"
  y += (5*SHORT(lnames[wbs->last]->height))/4;
  
  // CPhipps - patch drawing updated
  V_DrawMemPatch((320 - SHORT(finished->width))/2, y, 
...
OVERLAY void WI_drawEL(void)
{
  ...
  // draw "Entering"
  // CPhipps - patch drawing updated
  V_DrawMemPatch((320 - SHORT(entering->width))/2, y, FB, entering, NULL, VPT_STRETCH);

  // draw level
  y += (5*SHORT(lnames[wbs->next]->height))/4;

  // CPhipps - patch drawing updated
  V_DrawMemPatch((320 - SHORT(lnames[wbs->next]->width))/2, y, FB, 
		 lnames[wbs->next], NULL, VPT_STRETCH);
}
  ...
  const patch_t*  c[] )
{
  ...
  {
    // CPhipps - patch drawing updated
    V_DrawMemPatch(lnodes[wbs->epsd][n].x, lnodes[wbs->epsd][n].y,
...
OVERLAY static int WI_drawNum (int x, int y, int n, int digits)
{
  ...
  if (neg)
    // CPhipps - patch drawing updated
    V_DrawMemPatch(x-=8, y, FB, wiminus, NULL, VPT_STRETCH);
...
OVERLAY static void WI_drawPercent(int x, int y, int p)
{
  ...
  // CPhipps - patch drawing updated
  V_DrawMemPatch(x, y, FB, percent, NULL, VPT_STRETCH);
  WI_drawNum(x, y, p, -1);
}
...
{ 
  ...
  if (t < 100*60*60)
    for(;;) {
      n = t % 60;
      t /= 60;
      x = WI_drawNum(x, y, n, (t || n>9) ? 2 : 1) - SHORT(colon->width);

      // draw
      if (t)
	// CPhipps - patch drawing updated
        V_DrawMemPatch(x, y, FB, colon, NULL, VPT_STRETCH);
      else break;
    } 
  else // "sucks" (maybe should be "addicted", even I've never had a 100 hour game ;)
    V_DrawMemPatch(x - SHORT(sucks->width), y, FB, sucks, NULL, VPT_STRETCH); 
}
...
OVERLAY static void WI_drawTimeStats(int cnt_time, int cnt_total_time, int cnt_par)
{
  ...
  if (!(modifiedgame && !deh_pars))   
  {
    if (wbs->epsd < 3)
    {
      V_DrawMemPatch(320/2 + SP_TIMEX, SP_TIMEY, FB, par, NULL, VPT_STRETCH);
...
OVERLAY void WI_drawDeathmatchStats(void)
{
  ...
  V_DrawMemPatch(DM_TOTALSX-SHORT(total->width)/2,
		 DM_MATRIXY-WI_SPACINGY+10, FB, total, NULL, VPT_STRETCH);
  
  V_DrawMemPatch(DM_KILLERSX, DM_KILLERSY, FB, killers, NULL, VPT_STRETCH);
  V_DrawMemPatch(DM_VICTIMSX, DM_VICTIMSY, FB, victims, NULL, VPT_STRETCH);
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
  {
    if (playeringame[i]) {
      ...
      V_DrawMemPatch(x-SHORT(facebackp->width)/2, DM_MATRIXY - WI_SPACINGY, 
		     FB, facebackp, trans ? translationtables + 256*(trans-1) : NULL, 
		     VPT_STRETCH | (trans ? VPT_TRANS : 0));
      V_DrawMemPatch(DM_MATRIXX-SHORT(facebackp->width)/2, y, 
		     FB, facebackp, trans ? translationtables + 256*(trans-1) : NULL, 
		     VPT_STRETCH | (trans ? VPT_TRANS : 0));
...
OVERLAY void WI_drawNetgameStats(void)
{
  ...
  // draw stat titles (top line)
  V_DrawMemPatch(NG_STATSX+NG_SPACINGX-SHORT(kills->width),
		 NG_STATSY, FB, kills, NULL, VPT_STRETCH);

  V_DrawMemPatch(NG_STATSX+2*NG_SPACINGX-SHORT(items->width),
		 NG_STATSY, FB, items, NULL, VPT_STRETCH);

  V_DrawMemPatch(NG_STATSX+3*NG_SPACINGX-SHORT(secret->width),
		 NG_STATSY, FB, secret, NULL, VPT_STRETCH);
  
  if (dofrags)
    V_DrawMemPatch(NG_STATSX+4*NG_SPACINGX-SHORT(frags->width),
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
  {
    ...
    x = NG_STATSX;
    V_DrawMemPatch(x-SHORT(facebackp->width), y, FB, facebackp, 
		   trans ? translationtables+256*(trans-1) : NULL, 
		   VPT_STRETCH | (trans ? VPT_TRANS : 0));

    if (i == me)
      V_DrawMemPatch(x-SHORT(facebackp->width), y, FB, star, NULL, VPT_STRETCH);
...
OVERLAY void WI_drawStats(void)
{
  ...
  V_DrawMemPatch(SP_STATSX, SP_STATSY, FB, kills, NULL, VPT_STRETCH);
  WI_drawPercent(320 - SP_STATSX, SP_STATSY, cnt_kills[0]);

  V_DrawMemPatch(SP_STATSX, SP_STATSY+lh, FB, items, NULL, VPT_STRETCH);
  WI_drawPercent(320 - SP_STATSX, SP_STATSY+lh, cnt_items[0]);

  V_DrawMemPatch(SP_STATSX, SP_STATSY+2*lh, FB, sp_secret, NULL, VPT_STRETCH);
...
// Returns: void
//
// CPhipps - modified for new wad lump handling. Tabularised many vars and
//  their corresponding lump names
static const struct {
  const char      lumpname[9];
  const patch_t **pp;
} wi_data_vars[] = {
  { "WIMINUS", &wiminus}, // minus sign
  { "WIPCNT", &percent }, // percent sign
  { "WIF", &finished   },
  { "WIENTER",&entering},
  { "WIOSTK", &kills },
  { "WIOSTS", &secret },
...
OVERLAY void WI_loadData(void)
{
  ...
  // cph - check that the data from any previous visit here has already been freed
  // FIXME this should be #ifdef RANGECHECK'ed in future, but until I'm certain this 
  // code is stable, let's check always
  if (lnames) {
    lprintf(LO_WARN, "WI_loadData: previous intermission screen data not released\n");
    WI_unloadData();
  }
  ...
  {
    NUMCMAPS = 32;                
    lnames = (const patch_t **) Z_Malloc(sizeof(patch_t*) * NUMCMAPS,
					 PU_STATIC, 0);
  ...
  {
    lnames = (const patch_t **) Z_Malloc(sizeof(patch_t*) * NUMMAPS,
      PU_STATIC, 0);
    for (i=0 ; i<NUMMAPS ; i++)
    {
...
OVERLAY void WI_unloadData(void)
{
  ...
  if (gamemode == commercial) {
    ...
    W_UnlockLumpName("WIURH0");
    W_UnlockLumpName("WIURH1");
    W_UnlockLumpName("WISPLAT"); 
  ...
  Z_Free(lnames); lnames = NULL; // cph - paranoia - prevent repeat of the premature-lnames-free bug

  // CPhipps - table driven initialisation of misc patch vars
  for (i=0; wi_data_vars[i].pp; i++) 
    W_UnlockLumpName(wi_data_vars[i].lumpname);
